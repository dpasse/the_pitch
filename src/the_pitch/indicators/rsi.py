...
class RSI(AbstractIndicator):
    ...
    def compute(self, df: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        df['down_day'] = df['diff_close_1'].transform(
            lambda x : np.where(x < 0.0, x.abs(), 0.0)
        )
        ...
        df[self.name] = np.where(relative_strength_index == 0, Decimal('100'), (100 - (100 / (1 + relative_strength_index))).map(utils.decimal_from_float))
